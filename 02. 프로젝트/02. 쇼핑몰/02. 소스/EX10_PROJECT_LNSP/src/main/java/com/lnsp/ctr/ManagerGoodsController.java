...
public class ManagerGoodsController {
	...
	public String goods_delete(Model model, int idx, int status, int g_condition) {
		...
		return "manager_productlist.do?idx=" + idx + "&status=" + status +"&g_condition=" + g_condition;
		
	}
	
	@RequestMapping("/goods_soldout.do")
	public String goods_soldout(Model model, int idx, int status, int g_condition) {
	...
	public String goods_insert(Model model, int idx, int status, int g_condition) {
	...
	@RequestMapping("/manager_productsearch.do")
	public String manager_search(Model model, String search, Integer page, String text, String radio, int select) {
...
//		}
		...
		String pageMenu = Paging_Productlist.getPaging("manager_productsearch.do?search="+ search + "&radio=" + radio + 
				"&select=" + select + "&", nowPage, row_total , Common.Test.BLOCKLIST, Common.Test.BLOCKPAGE);
		
		List<GoodsVO> search_list = mg_goodsdao.Search(map);
		List<GoodsVO> list = new ArrayList<GoodsVO>();
		for (int i = start-1; i < end-1; i++) {
			if(i < search_list.size())
			list.add(search_list.get(i));
		}
