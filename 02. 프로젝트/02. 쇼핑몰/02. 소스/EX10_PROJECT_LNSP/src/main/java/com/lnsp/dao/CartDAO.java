...
public class CartDAO {
	...
	//장바구니 상품조회
	public List<CartVO> cart_list(int idx) {
		List<CartVO> list = sqlSession.selectList("cart.cart_list", idx);

		return list;
	}
	
	//장바구니 수량 업데이트
	public int update(int count, String g_num) {
		
		HashMap<String, Object> map = new HashMap<String, Object>();
		map.put("count", count);
		map.put("g_num", g_num);
		int cnt = sqlSession.update("cart.cart_update", map);
	...
	public CartVO selectOne(CartVO vo) {
		CartVO one = sqlSession.selectOne("cart.cart_one", vo);
		return one;
	}
	...
	//장바구니 상품삭제
	public CartVO selectOne(int idx, int m_num) {
		...
		vo = sqlSession.selectOne("cart.delete_cart", map);
		return vo;
	}


	//카트페이지 상품 수량 업데이트
	public int update_cart(int count, String g_num, int m_num) {
	...
	public int selectCart(int count, String g_num, int m_num) {
		...
		map.put("g_num", g_num);
		map.put("m_num", m_num);
		int res = sqlSession.selectOne("cart.cart_cnt_update", map);
