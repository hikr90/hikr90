...
public class MainController {
	...
	public String login(HttpServletResponse response, String id, String pwd) throws IOException {
		...
		// 일반 유저 / 관리자 확인
		MemberVO user_check = (MemberVO) (session.getAttribute("user"));

		if (user_check.getM_id().equals("admin")) {
			return "forward:manager.do"; // 관리자 전용 페이지로 이동
			// redirect 사용시 버그발생
			// 아마 Session관련으로 작동오류로 보임
		}
	...
	public String check_id(String m_id) {
		...
		String resultStr = String.format("[{'result' : '%s'}, {'m_id':'%s'}]", res, m_id);
	...
	// 아이디 찾기
		...
		@ResponseBody
		@RequestMapping("/searchID_action.do")
		public String searchID_action(Model model, String m_name, String m_phone) {
			HashMap<String, String> map = new HashMap<String, String>();
			map.put("m_name", m_name);
			map.put("m_phone", m_phone);
			int res = member_dao.check_memberid(map);
			
			String m_id = "";
			if( res == 1 ) {
				m_id = member_dao.get_memberid(map);
			}
		...
		@ResponseBody
		@RequestMapping("/searchPW_action.do")
		public String searchPW_action(Model model, String m_id, String m_email) {
			HashMap<String, String> map = new HashMap<String, String>();
			map.put("m_id", m_id);
			map.put("m_email", m_email);
			int res = member_dao.check_memberpwd(map);
			
			String m_pwd = "";
			if( res == 1 ) {
				m_pwd = member_dao.get_memberpwd(map);
			}
